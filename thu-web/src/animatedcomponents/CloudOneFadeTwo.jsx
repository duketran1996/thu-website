...
const Container = Keyframes.Spring(async next => {
  ...
  //   to: { opacity: 0.5, radians: 1 * Math.PI }
  // });

  while (true) {
    await next({
      from: { opacity: 0.5, radians: 0, color: "#F5B7B1" },
      to: { opacity: 1, radians: 0.5 }
    });
    await next({
      from: { opacity: 1, radians: 0.5, color: "#F5B7B1" },
      to: { opacity: 0.5, radians: 0 }
    });
  }
});

class CloudOneFadeTwo extends React.PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      show: true,
...
CloudOneFadeTwo.propTypes = {
  classes: PropTypes.object.isRequired
};

export default withStyles(styles)(CloudOneFadeTwo);
