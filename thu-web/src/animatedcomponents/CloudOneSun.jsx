...
const Container = Keyframes.Spring(async next => {
  while (true) {
    await next({
      from: { opacity: 0.2, radians: 0, color: "#5DADE2" },
      to: { opacity: 1, radians: 2 * Math.PI }
    });
    await next({
      from: { opacity: 1, radians: 0, color: "#5DADE2" },
      to: { opacity: 0.2, radians: 2 * Math.PI }
    });
  }
});

class CloudOneSun extends React.PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      show: true,
...
CloudOneSun.propTypes = {
  classes: PropTypes.object.isRequired
};

export default withStyles(styles)(CloudOneSun);
