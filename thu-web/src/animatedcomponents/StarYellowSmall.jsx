...
const Container = Keyframes.Spring(async next => {
  while (true) {
    ...
    await next({
      from: { opacity: 0.5, color: "#FEF9E7" },
      to: { opacity: 1, color: "#F9E79F" }
    });
  }
});

class StarYellowSmall extends React.PureComponent {
  render() {
    ...
    const Content = ({ opacity, color }) => (
      ...
      >
        <animated.defs>
          ...
          <animated.g id="star_yellow_small" fillOpacity={opacity} fill={color}>
            <path
        ...
        <use xlinkHref="#star_yellow_small" />
        <use
          xlinkHref="#star_yellow_small"
          filter="url(#glow_star_yellow_small)"
        />
      </animated.svg>
    );
    return (
...
StarYellowSmall.propTypes = {
  classes: PropTypes.object.isRequired
};

export default withStyles(styles)(StarYellowSmall);
