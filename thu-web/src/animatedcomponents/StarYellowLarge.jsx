...
const Container = Keyframes.Spring(async next => {
  while (true) {
    await next({
      from: { opacity: 0.5, color: "#FFE800" },
      to: { opacity: 1, color: "#FFF486" }
    });
    await next({
      from: { opacity: 1, color: "#FFF486" },
      to: { opacity: 0.5, color: "#FFE800" }
    });
    await next({
      from: { opacity: 1, color: "#FFF486" },
      to: { opacity: 0.5, color: "#FFE800" }
...
class StarYellowLarge extends React.PureComponent {
  render() {
    ...
    const Content = ({ opacity, color }) => (
      ...
      >
        <animated.defs>
          ...
          <animated.g id="star_yellow_large" fillOpacity={opacity} fill={color}>
            <path
        ...
        <use xlinkHref="#star_yellow_large" />
        <use
          xlinkHref="#star_yellow_large"
          filter="url(#glow_star_yellow_large)"
        />
      </animated.svg>
    );
    return (
      <div className={classes.neon}>
        <CssBaseline />
        <Container reset native config={{ duration: 500 }}>
          {Content}
        </Container>
      </div>
    );
...
StarYellowLarge.propTypes = {
  classes: PropTypes.object.isRequired
};

export default withStyles(styles)(StarYellowLarge);
